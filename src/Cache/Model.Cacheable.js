define(function (require){
	...
	return Backbone.Model.extend(_.extend({
	    ...
	    serverDelete: function (options){
		    ...
		    // if(this.collection) {
		    // 	return this.collection.serverDelete(this);
		    // }
	    ...
	    crud: function (method, dataFormat, urlFormat, options){
		    ...
		    _.extend(options, {
		        success: function (response, status, xhr){
			        ...
			        if (method !== 'read') {
			        	App.CacheManager.invalidateDependencies(method, model.service);
			        }
		    ...
		    return this.deferred = Backbone.sync.call(model, method, model, options);
	    }
	}, Cacheable));
});
