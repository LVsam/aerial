		...
		crud: function(method, dataFormat, urlFormat, options, model) {
			...
			_.extend(options, {
				success: function(resp, status, xhr) {
					...
					if(method !== 'read') {
						App.CacheManager.invalidateDependencies(method, collection.service);
					}
					if(!result){
						return result;
					}
				...
				error: Backbone.wrapError(error, collection, options)
			});
			return this.deferred = Backbone.sync.call(collection, method, collection, options);
		}
	}, Cacheable));
});